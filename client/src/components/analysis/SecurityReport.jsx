...
const SEVERITY_COLORS = {
  critical: { bg: 'bg-red-100', text: 'text-red-400', border: 'border-red-300', badge: 'bg-red-600' },
  high: { bg: 'bg-orange-100', text: 'text-orange-800', border: 'border-orange-300', badge: 'bg-orange-500' },
  medium: { bg: 'bg-amber-100', text: 'text-amber-400', border: 'border-amber-300', badge: 'bg-amber-900/150' },
  low: { bg: 'bg-blue-100', text: 'text-blue-400', border: 'border-blue-300', badge: 'bg-blue-400' },
};
...
function SeverityBar({ summary }) {
  ...
  if (total === 0) {
    return (
      <div className="flex items-center gap-2 text-primary-400 bg-primary-500/10 px-4 py-3 rounded-xl border border-surface-300">
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      ...
      {summary.medium > 0 && <span className="px-3 py-1.5 bg-amber-900/150 text-white rounded-full text-sm font-bold">{summary.medium} Medium</span>}
  ...
  return (
    <div className={`border rounded-xl overflow-hidden ${colors.border} shadow-sm`}>
      <button
        ...
        className="w-full text-left px-4 py-3.5 flex items-start gap-3 hover:bg-surface-100/50 transition-colors"
      >
