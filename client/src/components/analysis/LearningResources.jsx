function SourceLink({ url, label, icon, verified }) {
  ...
  return (
    ...
    >
      ...
      {verified !== undefined && (
        <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${verified ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
...
function ResourceCard({ resource }) {
  ...
  const confidenceColor = confidencePct >= 90 ? 'bg-green-500' : confidencePct >= 70 ? 'bg-blue-500' : 'bg-yellow-500';

  return (
    <div className="bg-white/80 border border-emerald-200 rounded-xl p-5 hover:shadow-md transition-shadow">
      ...
      {scrapeDetails?.githubStars != null && (
        <div className="flex items-center gap-1.5 mb-3 text-sm">
          ...
          <span className="text-gray-700 font-semibold">{scrapeDetails.githubStars.toLocaleString()}</span>
      ...
      <div className="space-y-2 mb-3">
        ...
        {resource.scrapeDetails?.youtubeTitle && (
          <div className="flex items-center gap-2 text-xs text-gray-400 mt-0.5 ml-6">
      ...
      {keyConcepts?.length > 0 && (
        <div className="flex flex-wrap gap-1.5 pt-3 border-t border-gray-100">
          {keyConcepts.map((concept) => (
            <span key={concept} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-md">
              {concept}
            </span>
          ))}
        </div>
...
function ConceptCloud({ concepts }) {
  ...
  return (
    <div className="bg-white/80 border border-emerald-200 rounded-xl p-5">
      ...
      <div className="flex flex-wrap gap-2">
        {sorted.slice(0, 20).map(([concept, techs]) => {
          ...
          return (
            <span
              key={concept}
              className={`${fontSize} bg-blue-50 text-blue-700 px-2.5 py-1 rounded-full cursor-default`}
              style={{ opacity }}
    ...
    <div className="bg-[#C9EDDC] rounded-2xl shadow-sm border border-emerald-200 p-6">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400">{totalResources} resources</span>
        </div>
      </div>
      ...
      {scraperErrors?.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 text-amber-700 px-3 py-2 rounded-lg mb-4 text-xs">
